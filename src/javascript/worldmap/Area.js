...
export default class Area extends EventEmitter {
  ...
  setFence() {
    ...
    // Geometry
    this.fence.geometry = new AreaFenceBufferGeometry(
      this.halfExtents.x * 2,
      this.halfExtents.y * 2,
      this.fence.depth
    );
  ...
  setKey() {
    ...
    this.key.enter.geometry = new THREE.PlaneBufferGeometry(
      this.key.enter.size,
    ...
    this.key.icon.geometry = new THREE.PlaneBufferGeometry(
      this.key.icon.size,
      this.key.icon.size,
      1,
      1
  ...
  interact(_showKey = true) {
    ...
    TweenLite.to(this.fence.mesh.position, 0.05, {
      z: 0,
      onComplete: () => {
        TweenLite.to(this.fence.mesh.position, 0.25, {
          z: 0.5,
  ...
  in(_showKey = true) {
    ...
    // Fence
    TweenLite.killTweensOf(this.fence.mesh.position);
    TweenLite.to(this.fence.mesh.position, 0.35, {
      z: this.fence.offset,
    ...
    if (this.hasKey) {
      ...
      // Animate
      if (_showKey) {
        TweenLite.to(this.key.container.position, 0.35, {
          z: this.key.shownZ,
          ease: Back.easeOut.config(3),
          delay: 0.1
        });
        TweenLite.to(this.key.icon.material, 0.35, {
          opacity: 0.5,
          ease: Back.easeOut.config(3),
          delay: 0.1
        });
        TweenLite.to(this.key.enter.material, 0.35, {
          opacity: 0.5,
          ease: Back.easeOut.config(3),
          delay: 0.1
        });
