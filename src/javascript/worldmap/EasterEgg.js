...
export default class EasterEggs {
  ...
  setKonamiCode() {
    ...
    this.konamiCode.label = new THREE.Mesh(
      new THREE.PlaneBufferGeometry(8, 8 / 16),
    ...
    this.konamiCode.testInput = (_input) => {
      ...
      if (
        this.konamiCode.sequence.toString() ===
        this.konamiCode.latestKeys.toString()
      ) {
        ...
        for (let i = 0; i < Math.pow(3, this.konamiCode.count); i++) {
          window.setTimeout(() => {
            const x =
              this.car.chassis.object.position.x + (Math.random() - 0.5) * 10;
            const y =
              this.car.chassis.object.position.y + (Math.random() - 0.5) * 10;
  ...
  setWigs() {
    ...
    this.wigs.change = () => {
      ...
      let randomWig = null;
      do {
        randomWig = this.wigs.items[
          Math.floor(Math.random() * this.wigs.items.length)
        ];
      ...
      // Set random material
      const randomMaterial = this.wigs.materials[
        Math.floor(Math.random() * this.wigs.materials.length)
      ];
    ...
    // Label
    this.resources.items.areaQuestionMarkTexture.magFilter =
      THREE.NearestFilter;
  ...
  setEggs() {
    ...
    this.eggs.add = (_options) => {
      ...
      egg.object = this.objects.add({
        ...
        rotation:
          typeof _options.sleep !== "undefined"
            ? _options.rotation
...
Would you like to go on the subscription page?
              ...
              window.open(
                `https://threejs-journey.xyz/subscribe/${code}`,
                "_blank"
              );
            }
      ...
      egg.object.collision.body.addEventListener(
        "collide",
        egg.collisionCallback
      );
